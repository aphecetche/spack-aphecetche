...
class PyTensorflow(Package, CudaPackage):
    ...
    depends_on('py-numpy@1.19.2:1.19.99',  type=('build', 'run'), when='@2.4:')
    ...
    if sys.byteorder == 'little':
        ...
        depends_on('py-grpcio@1.34.0:', type=('build', 'run'), when='@2.3:')

    depends_on('flatbuffers+python',type=('build','run'), when='@2.4:')
    ...
    def setup_build_environment(self, env):
        ...
        if self.spec.satisfies('@:2.3'):
            # scratch that as it does seem to break compilation for (at least) 
