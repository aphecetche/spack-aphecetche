...
class Php(AutotoolsPackage):
    ...
    url = "https://www.php.net/distributions/php-7.4.6.tar.gz"

    version('7.4.6',     sha256='2a37bab4e308c4e3867083137b7cce4a3f1d996ae231b383c1a83609cec3fed0')
    ...
    depends_on('libiconv', type='build', when='+iconv')
    depends_on('libxml2')
    depends_on('sqlite')
    ...
    variant(
        'iconv',
        default=True,
        description='include libiconv support')
   
    def configure_args(self):
        args = [  ]
        if '+iconv' in self.spec:
            args.append('--with-iconv={0}'.format(self.spec['libiconv'].prefix))
        return args
